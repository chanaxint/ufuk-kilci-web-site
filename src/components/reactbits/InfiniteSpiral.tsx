/*
 * React Bits — InfiniteSpiral
 * Projeye özel değişiklik: kartlar yalnızca görsel değil, serbest içerik de
 * (`node`) alabiliyor. Yorum kartlarını spiralde göstermek için eklendi.
 */
import {
  useEffect,
  useMemo,
...
export interface InfiniteSpiralItem {
  id?: string | number;
  /** Kart içeriği görsel yerine serbest düğüm olabilir */
  node?: ReactNode;
  src?: string;
  alt?: string;
  href?: string;
...
export interface InfiniteSpiralProps {
  ...
  className?: string;
  /**
   * Projeye özel: ilerlemeyi dışarıdan süren referans (madde cinsinden).
   * Verildiğinde bileşenin kendi scroll dinleyicisi ve otomatik dönüşü
   * devre dışı kalır; kaç maddenin döneceğini tamamen çağıran belirler.
   */
  externalProgress?: RefObject<number>;
}
...
const InfiniteSpiral = ({
  ...
  useEffect(() => {
    ...
    const scrollEnabled =
      !externalProgress && (animationMode === 'scroll' || animationMode === 'all');
    ...
    const render = (time: number) => {
      ...
      previousTime = time;
      if (externalProgress) targetProgressRef.current = externalProgress.current;
      const autoEnabled =
        !externalProgress && (animationMode === 'auto' || animationMode === 'all');
      ...
      <div className="absolute inset-0 [transform-style:preserve-3d]" role="list" aria-label="Infinite spiral gallery">
        {normalizedItems.map((item, index) => {
          const content = item.node ? (
            <div className="absolute inset-0 flex h-full w-full select-none flex-col">{item.node}</div>
          ) : (
            <img
