/*
 * 21st.dev — "Draw Random Underline" (Osmo kaynaklı)
 *
 * Projeye özel değişiklikler:
 *  - Özgün demo kendi CSS sınıflarıyla (text-draw, text-draw__box …) geliyordu;
 *    burada stiller Tailwind'e ve sitenin tipografisine taşındı.
...
type PathData = { d: string; viewBox: string }

/** Elle çizilmiş altı ayrı alt çizgi; sırayla dolaşılır. */
const VARIANTS: PathData[] = [
  {
...
/*
 * Özgün koddaki `let nextIndex = null` ile aynı iş: ilk üzerine gelmede
 * rastgele başlar, sonrasında bütün bağlantılar tek bir sırayı paylaşır.
 */
let nextIndex: number | null = null
const takeVariant = (): PathData => {
  nextIndex =
    nextIndex === null
      ? Math.floor(Math.random() * VARIANTS.length)
      : (nextIndex + 1) % VARIANTS.length
...
export default function DrawUnderlineLink({
  text,
  href,
  stroke = '#8a5a33',
  className = '',
  ...
  const [variant, setVariant] = useState<PathData | null>(null)
  const drawIn = useRef<gsap.core.Tween | null>(null)
  const drawOut = useRef<gsap.core.Tween | null>(null)

  /* Yeni bir çizgi seçildiğinde soldan sağa çizilir */
  useEffect(() => {
    const path = pathRef.current
    if (!path || !variant) return

    if (drawOut.current?.isActive()) {
      drawOut.current.kill()
      drawOut.current = null
    }
  ...
  const show = () => {
    if (drawIn.current?.isActive()) return
    setVariant(takeVariant())
  }
  ...
  const hide = () => {
    ...
    const start = () => {
      if (drawOut.current?.isActive()) return
      drawOut.current = gsap.to(path, {
        duration: 0.5,
        drawSVG: '100% 100%',
        ease: 'power2.inOut',
        onComplete: () => {
    ...
    /* Çizim sürüyorsa bitmesini bekle, yarıda kesip zıplatma */
    if (drawIn.current?.isActive()) drawIn.current.eventCallback('onComplete', start)
    else start()
  }
