...
| `StaggeredMenu` | Mobil menü | `showHeader` ve `controlledOpen` eklendi (site kendi navigasyon çubuğunu kullanıyor). İlk kurulum efektindeki erken dönüş, başlık gizlendiğinde paneli ekran dışına itmeyi atlıyordu (menü açılışta açık görünüyordu) — koşul yalnızca panele bağlandı. Tipografi, renkler ve numaralandırma projeye uyarlandı. |
